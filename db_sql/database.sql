...
CREATE TABLE `users` (
  ...
  `role` int(1) NOT NULL DEFAULT 0 COMMENT '権限 (0:一般, 1:管理者, 2:フロント, 3:清掃)',
  `notes` text DEFAULT NULL COMMENT '特記事項',
  `reset_token` varchar(64) DEFAULT NULL COMMENT 'パスワードリセット用トークン',
  `reset_expires_at` datetime DEFAULT NULL COMMENT 'パスワードリセットトークン有効期限',
...
CREATE TABLE `bookings` (
  ...
  `reminder_sent` tinyint(1) NOT NULL DEFAULT 0 COMMENT 'リマインダーメール送信済みフラグ',
  `thankyou_sent` tinyint(1) NOT NULL DEFAULT 0 COMMENT 'サンキューメール送信済みフラグ',
  `created_at` datetime NOT NULL DEFAULT current_timestamp(),
  `updated_at` datetime NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp(),
  PRIMARY KEY (`id`),
  KEY `user_id` (`user_id`),
